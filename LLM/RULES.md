...
3. **Zero-Trust & Ngoại lệ (Edge Cases):** Mặc định code mới luôn có nguy cơ chứa lỗi hoặc hổng bảo mật. Phân tích và xử lý triệt để các trường hợp dữ liệu rỗng, sai định dạng hoặc ngoại lệ để NGĂN CHẶN/LOẠI BỎ nguy cơ sập hệ thống. Tuyệt đối không chỉ code theo "happy path".
4. **Kỹ thuật Hỏi ngược (Proactive Questioning):** **NẾU** ngữ cảnh cung cấp còn thiếu sót, mơ hồ hoặc có nhiều cách hiểu. NÊN chủ động đặt câu hỏi ngược lại (Ví dụ: "Hãy cung cấp 3 thông tin sau để tôi hoàn thành với độ chính xác 95% ...") để làm rõ vấn đề, hạn chế hành vi tự ý giả định, trước khi đưa ra giải pháp.
